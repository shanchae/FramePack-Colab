...
def worker(input_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, mp4_crf):
    ...
    for idx, run_seed in enumerate(seeds):
        try:
            ...
            # Text encoding
            stream.output_queue.push(('progress', (None, '', make_progress_bar_html(0, f'Text encoding ... [Seed {run_seed}]'))))
            if not high_vram:
            ...
            stream.output_queue.push(('progress', (None, '', make_progress_bar_html(0, f'Image processing ... [Seed {run_seed}]'))))
            ...
            # VAE encoding
            stream.output_queue.push(('progress', (None, '', make_progress_bar_html(0, f'VAE encoding ... [Seed {run_seed}]'))))
            if not high_vram:
                load_model_as_complete(vae, target_device=gpu)
            start_latent = vae_encode(input_image_pt, vae)

            # CLIP Vision
            stream.output_queue.push(('progress', (None, '', make_progress_bar_html(0, f'CLIP Vision encoding ... [Seed {run_seed}]'))))
            if not high_vram:
            ...
            # Sampling
            stream.output_queue.push(('progress', (None, '', make_progress_bar_html(0, f'Start sampling ... [Seed {run_seed}]'))))
            rnd = torch.Generator("cpu").manual_seed(run_seed)
